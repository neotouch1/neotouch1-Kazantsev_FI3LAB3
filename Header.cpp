...
{
	for (int i = 0; i < length - 1; i++)	if (string[i] == n) return i;
	return -1;
}
...
TString TString::operator+(const TString& p)
{
	...
	for (int i = 0; i < length - 1; i++)	
		A.string[i] = string[i];
	for (int j = length - 1; j < len; j++) 
		A.string[j] = p.string[j - length + 1];
	return A;
}
